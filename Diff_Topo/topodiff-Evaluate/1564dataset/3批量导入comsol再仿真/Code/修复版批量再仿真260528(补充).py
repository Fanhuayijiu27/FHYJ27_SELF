# -*- coding: utf-8 -*-
"""
批量导入 COMSOL 边界点并重新仿真（补跑缺失温度场 TXT 版本）

功能流程：
1. 先扫描主输出目录中已有的 模型结构导入comsol仿真温度场_000.txt ~ _099.txt
2. 自动识别缺失的序号
3. 仅对缺失序号重新执行原批量 COMSOL 流程
4. 每次仍从干净的源 mph 文件重新打开模型
5. 根据名字对照表修改 widhr1、vocc1、widhr2、vocc2
6. 按 boundary_curve_1.txt ~ boundary_curve_X.txt 生成 pol1 ~ polX
7. 构建几何、划分网格
8. 检查材料 mat2 与层流 spf 当前选中域，并取消多余域
9. 运行 sol1
10. 补跑 TXT 输出到 再仿真温度场(补充不收敛)/txt
11. 补跑 MPH 输出到 再仿真温度场(补充不收敛)/mph
...
MPH_SOURCE = Path(
    r"D:\Diff_Topo\开源\topodiff-Evaluate\1564dataset\3批量导入comsol再仿真\Benchmark\模型结构导入comsolBench副本.mph"
)

BOUNDARY_ROOT = Path(
    r"D:\Diff_Topo\开源\topodiff-Evaluate\1564dataset\2分块输出边界点"
)

NAME_TABLE = Path(
    r"D:\Diff_Topo\开源\topodiff-Evaluate\1564dataset\1三列数据二值化\名字对照表_modified.txt"
)

EXPORT_ROOT = Path(
    r"D:\Diff_Topo\开源\topodiff-Evaluate\1564dataset\3批量导入comsol再仿真\再仿真温度场"
)

# 主输出目录：用于检测哪些温度场 TXT 已经存在。
# 注意：补跑结果不会写回这个目录，而是写到 SUPPLEMENT_ROOT 下。
MAIN_TXT_DIR = EXPORT_ROOT

# 补跑输出目录
SUPPLEMENT_ROOT = EXPORT_ROOT / "再仿真温度场(补充不收敛)"
SUPPLEMENT_TXT_DIR = SUPPLEMENT_ROOT / "txt"
MPH_SAVE_DIR = SUPPLEMENT_ROOT / "mph"

# 补跑日志
LOG_CSV = SUPPLEMENT_ROOT / "batch_comsol_supplement_log.csv"
DOMAIN_LOG_CSV = SUPPLEMENT_ROOT / "domain_selection_supplement_log.csv"
MISSING_CASES_TXT = SUPPLEMENT_ROOT / "missing_case_ids.txt"
DOMAIN_CHECK_TMP_DIR = SUPPLEMENT_ROOT / "_tmp_domain_check"
...
# 补跑目录中已有温度场 TXT 时是否跳过。
# 建议保持 True，这样脚本中断后重新运行时不会重复补跑已成功的缺失编号。
...
def ensure_dirs():
    ...
    MAIN_TXT_DIR.mkdir(parents=True, exist_ok=True)
...
def scan_existing_main_txt_ids():
    """
    扫描主输出目录 MAIN_TXT_DIR 中已经生成的温度场 TXT。

    只扫描 MAIN_TXT_DIR 当前层，不递归进入补跑目录，避免把补跑结果误认为主目录结果。
    """
    pattern = re.compile(r"^模型结构导入comsol仿真温度场_(\d{3})\.txt$", re.IGNORECASE)
    existing_ids = set()

    if not MAIN_TXT_DIR.exists():
        return existing_ids

    for path in MAIN_TXT_DIR.iterdir():
        if not path.is_file():
            continue
...
def find_missing_case_ids():
    """
    根据 START_ID ~ END_ID，找出主输出目录中缺失的温度场 TXT 序号。
    """
    expected_ids = [f"{i:03d}" for i in range(START_ID, END_ID + 1)]
    existing_ids = scan_existing_main_txt_ids()
    missing_ids = [case_id for case_id in expected_ids if case_id not in existing_ids]
    return expected_ids, sorted(existing_ids), missing_ids


def write_missing_case_report(expected_ids, existing_ids, missing_ids):
    """
    将缺失序号写入补跑目录，便于检查。
    """
    with open(MISSING_CASES_TXT, "w", encoding="utf-8-sig") as f:
        f.write("主输出目录温度场 TXT 缺失检查结果\n")
        f.write(f"主输出目录: {MAIN_TXT_DIR}\n")
        f.write(f"检查范围: {START_ID:03d} ~ {END_ID:03d}\n")
        f.write(f"理论数量: {len(expected_ids)}\n")
        f.write(f"主目录已存在数量: {len(existing_ids)}\n")
        f.write(f"缺失数量: {len(missing_ids)}\n\n")
        f.write("缺失序号:\n")
        if missing_ids:
...
def supplement_txt_path(case_id):
    case_id = normalize_case_id(case_id)
    return SUPPLEMENT_TXT_DIR / f"模型结构导入comsol仿真温度场_{case_id}.txt"


def supplement_mph_path(case_id):
    case_id = normalize_case_id(case_id)
    return MPH_SAVE_DIR / f"模型结构导入comsol再仿真_{case_id}.mph"
...
def process_one_case(client, case_id, param_mapping):
    ...
    # 补跑结果写入专门的补充目录，不覆盖主输出目录中的已有文件。
    export_txt = supplement_txt_path(case_id)
    save_mph_path = supplement_mph_path(case_id)
...
def main():
    ...
    print("[检查主输出目录中缺失的温度场 TXT]")
    expected_ids, existing_ids, missing_ids = find_missing_case_ids()
    write_missing_case_report(expected_ids, existing_ids, missing_ids)

    print(f"    主输出目录：{MAIN_TXT_DIR}")
    print(f"    理论数量：{len(expected_ids)}")
    print(f"    已存在数量：{len(existing_ids)}")
    print(f"    缺失数量：{len(missing_ids)}")

    if missing_ids:
        print("    缺失序号：" + ", ".join(missing_ids))
    else:
        print("    未发现缺失 TXT，无需补跑。")
        print(f"    缺失检查报告：{MISSING_CASES_TXT}")
        return
    ...
    print("[补跑结束]")
    print(f"缺失检查报告：{MISSING_CASES_TXT}")
    print(f"补跑 TXT 目录：{SUPPLEMENT_TXT_DIR}")
    print(f"补跑 MPH 目录：{MPH_SAVE_DIR}")
