...
NBossSideMillingCmd::NBossSideMillingCmd():
TaggedObject()
{
}

NBossSideMillingCmd::NBossSideMillingCmd(std::vector<NormalBoss*> ListOfNBoss, std::vector<NoSideBoss*> ListOfNSBoss, BossSideProcessParameter ParameterSet, Part* WorkPart, Body* WorkBody, Body * WorkBlank, std::vector<Face*>ListOfSelectedFace, std::vector<Face*>ListOfSelectedZhiDingFace) :
TaggedObject()
{
	...
	if (BossParameterSet.FaceSelect&&BossParameterSet.syspart != 0)
	{
		...
		{
			BossSideMillingDecision1(SelectedFaceList1,SelectedZhiDingFaceList);
		}
	}
...
Point3d NBossSideMillingCmd::GetCenterPoint(Face *face)
{
...
tag_t NBossSideMillingCmd::SelectPart()
{
	int count = 0, resp = 0;
	double cursor[3] = { 0 };
...
tag_t NBossSideMillingCmd::SelectBlank()
{
	int count = 0, resp = 0;
	double cursor[3] = { 0 };
	tag_t objTag = NULL_TAG, view = NULL_TAG;
...
Vector3d NBossSideMillingCmd::get_Centre_fr_Face(Face* ipFace)
{
...
double NBossSideMillingCmd::DotProduct(double a[3], double b[3])
{
	double result;
...
void NBossSideMillingCmd::BossSideMillingDecision1(std::vector<Face*> BossTopFaceList, std::vector<Face*> SelectedZhiDingface)
{
	...
	for (std::vector<Face*>::size_type i = 0; i != BossTopFaceList.size(); i++)
	{
		...
		std::vector<Face*> DiJiaoFace;
		std::vector<Face*>SideFace;
		for (std::vector<NormalBoss*>::size_type j = 0; j != BossResultList.size(); j++)
		{
			NormalBoss* EachBoss = BossResultList[j];
			Face* EachBossTopFace = EachBoss->TopFaceVector[0];
			if (TopFaceTag == EachBossTopFace->Tag() && (strstr(EachBoss->name, "含侧面凸台") != NULL))
			{
				...
				DiJiaoFace = EachBoss->BottomCornerFaceVector;
				SideFace = EachBoss->SideFaceVector;
			}
		}
		for (std::vector<NoSideBoss*>::size_type j = 0; j != BossResultList1.size(); j++)
		{
		...
		//{
		//	Face *dijiaoface = DiJiaoFace[i];
		//	std::vector<Edge*>dijiaofaceedge = dijiaoface->GetEdges();
		//	for (int j = 0; j < dijiaofaceedge.size(); j++)
		//	{
		//		Edge *edge11 = dijiaofaceedge[j];
		//		tag_t edge11tag = edge11->Tag();
		//		int edgetype;
		//		UF_MODL_ask_edge_type(edge11tag, &edgetype);
		...
		//UF_CURVE_arc_s * arcdata;
		//UF_CURVE_ask_arc_data(arctag, arcdata);
		//double daojiaoradius = arcdata->radius;

		//if (toolradius!=daojiaoradius)
		//{
		//	uc1601("请选择合适大小的刀具",1);
		//}
		...
		{
			Face *dijiaoface = DiJiaoFace[i];
			tag_t dijiaofacetag = dijiaoface->Tag();
		...
		for (int k = 0; k < edgevector.size(); k++)
		{
			...
			if (edgetype != UF_MODL_CIRCULAR_EDGE&&edgetype != UF_MODL_ELLIPTICAL_EDGE)
			{
				ifallcircle = 1;
				break;
			}
		...
		{
			tag_t edgetag = edgevector[i]->Tag();
			int edgetype;
			UF_MODL_ask_edge_type(edgetag, &edgetype);
		...
		//
		std::vector<NXOpen::Edge*>edgevector3;
		for (int i = 0; i < edgevector.size(); i++)
		{
		...
		//设置偏置曲线，通用性较好，比较依赖识别的准确性
		//获取凸台顶面的外轮廓
		//顶面与侧面的交线
		//std::vector<NXOpen::Edge*>WaiLunKuoedge;
		//for (int i = 0; i < edgevector.size();i++)
		//{
		//	int a = 0;
		//	for (int j = 0; j < SideFace.size();j++)
		//	{
		//		std::vector<NXOpen::Edge*>sidefaceedge = SideFace[j]->GetEdges();
		//		for (int k = 0; k < sidefaceedge.size();k++)
		//		{
		//			if (edgevector[i]->Tag()==sidefaceedge[k]->Tag())
		//			{
		//				a = 1;
		//				WaiLunKuoedge.push_back(edgevector[i]);
		//				break;
		//			}
		//		}
		//		if (a==1)
		...
		//{
		//	int a = 0;
		//	for (int j = 0; j < DiJiaoFace.size(); j++)
		//	{
		//		std::vector<NXOpen::Edge*>dijiaofaceedge = DiJiaoFace[j]->GetEdges();
		//		for (int k = 0; k < dijiaofaceedge.size(); k++)
		//		{
		//			if (edgevector[i]->Tag() == dijiaofaceedge[k]->Tag())
		//			{
		//				a = 1;
		//				WaiLunKuoedge.push_back(edgevector[i]);
		//				break;
		...
		//for (int i = 0; i < WaiLunKuoedge.size(); i++)
		//{
		//	std::vector<Edge*>edgevector2(1);
		//	edgevector2[0] = WaiLunKuoedge[i];
		...
		//	WaiLunKuoedge[i]->GetVertices(&helpPoint1, &helpPoint2);
		...
		////若为圆形凸台，偏置方向相反
		//if (WaiLunKuoedge.size()==1||WaiLunKuoedge.size()>15)
		//{
		...
		if (ifallcircle == 1)
		{
			if (ifhavelineedge)
			{
				if (type3004num == 0 || type3004num == 1 || type3004num == 2)
				{
					//按顺序存入边
					std::vector<NXOpen::Edge*>edgevector1;
					Edge *edge = edgevector[circleedge];
					edgevector1.push_back(edge);
					...
					/*for (int i = 0; i < edgevector.size(); i++)
					{
						...
						if (point1.X == point3.X&&point1.Y == point3.Y&&point1.Z == point3.Z)
						{
							point1 = point4;
							int a = 0;
							for (int j = 0; j < edgevector1.size(); j++)
							{
								if (edge1->Tag() == edgevector1[j]->Tag())
								{
									a = 1;
									break;
								}
					...
					for (int i = 0; i < edgevector1.size(); i++)
					{
						std::vector<Edge*>edgevector2(1);
						edgevector2[0] = edgevector1[i];
						NXOpen::EdgeDumbRule *edgeDumbRule1;
						edgeDumbRule1 = workPart->ScRuleFactory()->CreateRuleEdgeDumb(edgevector2);
						offsetCurveBuilder1->CurvesToOffset()->AllowSelfIntersection(true);
						std::vector<NXOpen::SelectionIntentRule *> rules1(1);
						...
						NXOpen::Point3d helpPoint1;
						NXOpen::Point3d helpPoint2;
						edgevector1[i]->GetVertices(&helpPoint1, &helpPoint2);
						offsetCurveBuilder1->CurvesToOffset()->AddToSection(rules1, edgevector2[0], nullNXOpen_NXObject, nullNXOpen_NXObject, helpPoint1, NXOpen::Section::ModeCreate, false);
					}

				}
				else if (type3004num == 4)
				{
					for (int i = 0; i < edgevector.size(); i++)
					{
						NXOpen::Point3d helpPoint1;
						...
						if (helpPoint1.X == helpPoint2.X && helpPoint1.Y == helpPoint2.Y && helpPoint1.Z == helpPoint2.Z)
						{
							continue;
						}
						else
				...
				{
					tag_t edgetag = edgevector[edgevector.size() - 1]->Tag();
					int edgetype;
					UF_MODL_ask_edge_type(edgetag, &edgetype);
					for (int i = 0; i < edgevector.size() - 1; i++)
					{
						NXOpen::Point3d helpPoint1;
						NXOpen::Point3d helpPoint2;
						edgevector[i]->GetVertices(&helpPoint1, &helpPoint2);
					...
					/*int max3004edge;
					double max3004length = 0;
					for (int j = 0; j < edgevector.size(); j++)
					{
						tag_t edgetag = edgevector[j]->Tag();
						int edgetype;
						UF_MODL_ask_edge_type(edgetag, &edgetype);

						if (edgetype == UF_MODL_INTERSECTION_EDGE)
						{
							if (edgevector[j]->GetLength() > max3004length)
							{
								max3004edge = j;
							}
						}
					...
					edges1[0] = edgevector[edgevector.size() - 1];
					NXOpen::EdgeDumbRule *edgeDumbRule1;
					...
					NXOpen::Point3d helpPoint1;
					NXOpen::Point3d helpPoint2;
					edgevector[edgevector.size() - 1]->GetVertices(&helpPoint1, &helpPoint2);
					offsetCurveBuilder1->CurvesToOffset()->AddToSection(rules1, edgevector[edgevector.size() - 1], nullNXOpen_NXObject, nullNXOpen_NXObject, helpPoint1, NXOpen::Section::ModeCreate, false);

					offsetCurveBuilder1->SetReverseDirection(true);

					if (edgetype == UF_MODL_LINEAR_EDGE)
					{
						offsetCurveBuilder1->SetPointOnOffsetPlane(midPoint);
						offsetCurveBuilder1->SetReverseDirection(false);
					}
					...
					{
						Section *section1 = workPart->Sections()->CreateSection(objects1[i]);
		...
		else
		{
			...
			{
				Edge *edge1 = edgevector[i];
				NXOpen::Point3d helpPoint11;
				NXOpen::Point3d helpPoint22;
				edge1->GetVertices(&helpPoint11, &helpPoint22);
				if (helpPoint11.X != helpPoint22.X || helpPoint11.Y != helpPoint22.Y || helpPoint11.Z != helpPoint22.Z)
				{
					a = 1;
					break;
				}
			...
			else
			{
				...
				edgevector1[0] = edgevector[maxedge];
				NXOpen::EdgeDumbRule *edgeDumbRule1;
				...
				NXOpen::Point3d helpPoint1;
				NXOpen::Point3d helpPoint2;
				edgevector[maxedge]->GetVertices(&helpPoint1, &helpPoint2);
