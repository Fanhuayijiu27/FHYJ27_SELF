...
void AnnulusCavityBottomMillingCmd::AnnulusCavityBottomMillingDecision4(std::vector<Face*> SelectedFaceList)
{
	...
	for (std::vector<AnnulusCavity*>::size_type i = 0; i != SelectedFaceList.size(); i++)
	{
		...
		//sprintf(message2, "%d\n", centeripoint.Y);
		//sprintf(message3, "%d\n", centeripoint.Z);
		//UF_UI_write_listing_window("zhongdian坐标:");
		...
		//sprintf(message5, "%d\n", wcsMatrixValues[7]);
		//sprintf(message6, "%d\n", wcsMatrixValues[8]);
		//UF_UI_write_listing_window("轴线坐标:");
		//UF_UI_write_listing_window(message4);
		//UF_UI_write_listing_window(message5);
		//UF_UI_write_listing_window(message6);

		//std::vector<Edge*> AllEdges = EachAnnulusCavity->GetEdges();

		//for (Edge *edge : AllEdges)
		//{

		//获取所有的边,边非孔
		std::vector<Edge*> AllEdges; //= iFace->GetEdges();
		AllEdges.clear();
		...
		for (int i = 0; i < loops_count; i++)
		{
			...
			if (loop_stype == 1)   //非空
			{
				...
				for (int j = 0; j < EdgeListCount; j++)
				{
					...
					AllEdges.push_back(pEdge_Each);//存入内环列表中
				}
			}
		...
		//获取上下边
		std::vector<Edge*> UDEdges; //= iFace->GetEdges();
		UDEdges.clear();
		for (Edge *edge : AllEdges)
		{

			tag_t tagEdge = edge->Tag();
			int iEdgeType = 0;
			UF_MODL_ask_edge_type(tagEdge, &iEdgeType);
			if (iEdgeType == UF_MODL_CIRCULAR_EDGE)
			{
				UDEdges.push_back(edge);
		...
		/*char message1234[13];
		UF_UI_open_listing_window();
		sprintf(message1234, "%d\n", UDEdges.size());
		UF_UI_write_listing_window("边个数1111:");
		UF_UI_write_listing_window(message1234);*/
		...
		string strToolName = AnnulusCavityParameterSet.ToolMsg.ToolName.GetLocaleText();
		...
		UF_PARAM_ask_double_value(toolTag, UF_PARAM_TL_COR1_RAD, &DIAMETERvalue);

		// 计算长度
		Edge *edge1 = UDEdges[0];
		Edge *edge2 = UDEdges[1];

		UF_EVAL_arc_s  arc1;
		UF_EVAL_arc_s  arc2;

		tag_t iEdge_tag1 = edge1->Tag();
		tag_t iEdge_tag2 = edge2->Tag();

		UF_EVAL_p_t evaluator1;
		UF_EVAL_initialize(iEdge_tag1, &evaluator1);
		UF_EVAL_p_t evaluator2;
		UF_EVAL_initialize(iEdge_tag2, &evaluator2);
		int err1 = UF_EVAL_ask_arc(evaluator1, &arc1);
		int err2 = UF_EVAL_ask_arc(evaluator2, &arc2);

		/*char buffer[50];

		sprintf(buffer, sizeof(buffer), "%.2f", arc1.radius);*/

		double ra = abs(float(arc1.radius) - float(arc2.radius));

		double height = sqrt(pow((float(arc1.center[0]) - float(arc2.center[0])), 2) +
						pow((float(arc1.center[1]) - float(arc2.center[1])), 2) +
						pow((float(arc1.center[2]) - float(arc2.center[2])), 2));

		double length_ = sqrt(pow(ra, 2) + pow(height, 2));
		UF_EVAL_free(evaluator1);
		UF_EVAL_free(evaluator2);

		//char message0[13];
		//UF_UI_open_listing_window();
		//sprintf(message0, "%.2f", ra);
		//UF_UI_write_listing_window("半径差:");
		//UF_UI_write_listing_window(message0);

		//char message1[13];
		//UF_UI_open_listing_window();
		//sprintf(message1, " %.2f", height);
		//UF_UI_write_listing_window("高度差:");
		//UF_UI_write_listing_window(message1);

		//char message133[13];
		//UF_UI_open_listing_window();
		//sprintf(message133, "%.2f", length_);
		//UF_UI_write_listing_window("宽度:");
		//UF_UI_write_listing_window(message133);

		//jisuan s e
		float part1 = AnnulusCavityParameterSet.ToolMsg.ToolDiameter - 2 * DIAMETERvalue;
		float part2 = part1 / 2;
		float part3 = part2 / length_;
		float result11 = part3 * 100;
		float start_p = round(10000 * result11) / 10000;
		float end_p = 100 - start_p;
		//jisuan bujushu

		float A4 = length_;
		float B4 = DIAMETERvalue;
		float C4 = AnnulusCavityParameterSet.ToolMsg.ToolDiameter;

		float step1 = C4 - 2 * B4;               // C4 - 2 * B4
		float step2 = A4 - step1;                // A4 - (C4 - 2 * B4)
		float step3 = step1 * 0.90;              // (C4 - 2 * B4) * 0.90
		float result22 = step2 / step3;            // (A4 - (C4 - 2 * B4)) / ((C4 - 2 * B4) * 0.90)

		//// 向上取整
		int roundedResult22 = ceil(result22);


		/*char message1333[13];
		UF_UI_open_listing_window();
		sprintf(message1333, " 刀具圆角半径= %.4f", DIAMETERvalue);
		UF_UI_write_listing_window("qishidasdasd:");
		UF_UI_write_listing_window(message1333);*/

		/*char message13334[13];
		UF_UI_open_listing_window();
		sprintf(message13334, "R = %.6f", part1);
		UF_UI_write_listing_window("456465:");
		UF_UI_write_listing_window(message13334);

		char message1333333[13];
		UF_UI_open_listing_window();
		sprintf(message1333333, "R = %.6f", part2);
		UF_UI_write_listing_window("78798798798:");
		UF_UI_write_listing_window(message1333333);

		char message1333888[13];
		UF_UI_open_listing_window();
		sprintf(message1333888, "R = %.6f", part3);
		UF_UI_write_listing_window("qishidasdasd:");
		UF_UI_write_listing_window(message1333888);*/

		//char message1333[13];
		//UF_UI_open_listing_window();
		//sprintf(message1333, "R = %.6f", start_p);
		//UF_UI_write_listing_window("qishidasdasd:");
		//UF_UI_write_listing_window(message1333);
		...
		//char msg12[133];
		//UF_UI_write_listing_window(msg12);
		//UF_UI_write_listing_window("\n");
		//sprintf(msg12, "（半径）：R=%.2f", arc2.radius);
		if (UDEdges.size()==2)
		{
			surfaceContourBuilder1->DmSurfBuilder()->SetSurfacePercentageStartStep(start_p);

			surfaceContourBuilder1->DmSurfBuilder()->SetSurfacePercentageEndStep(end_p);
		}
		else
		{
			surfaceContourBuilder1->DmSurfBuilder()->SetSurfacePercentageStartStep(0);

			surfaceContourBuilder1->DmSurfBuilder()->SetSurfacePercentageEndStep(100);
		}
		...
		if (AnnulusCavityParameterSet.StepDisAmount==0)
		{
			surfaceContourBuilder1->DmSurfBuilder()->StepoverBuilder()->SetNumberOfStepovers(roundedResult22);
		}
		else
		{
