...
RoughPlungingCmd::RoughPlungingCmd(std::vector<CircularGroove*> ListOfCavity, std::vector<SubsidenceCavity*> ListOfSubsidenceCavity, std::vector<ArcCavity*> ListOfArcCavity, 
								   ...
								   std::vector<Face*> ListOfSelectedFace, tag_t partTag, tag_t blankTag) :
TaggedObject()
{
...
void RoughPlungingCmd::GeneralRoughPlungingDecision(std::vector<Face*> ListOfSelectedFace)
{
	...
	for (std::vector<Face*>::size_type i = 0; i != SelectedFaceList.size(); i++)
	{
		...
		if (type != 22)
		{
			...
			sprintf(newvector, "临时向量和单位切向量：\n%f, %f, %f,\n%f, %f, %f\n",
				tempvector[0], tempvector[1], tempvector[2],
				tagentVector[0], tagentVector[1], tagentVector[2]);
			UF_UI_write_listing_window(newvector);*/
			...
			/*char MyMCS[256];
			UF_UI_open_listing_window();
			sprintf(MyMCS, "调整后的MCS矩阵为：\n%f, %f, %f,\n%f, %f, %f, \n%f, %f, %f\n\n",
				targetMTx[0], targetMTx[1], targetMTx[2],
				targetMTx[3], targetMTx[4], targetMTx[5],
				targetMTx[6], targetMTx[7], targetMTx[8]);
			UF_UI_write_listing_window(MyMCS);*/
		...
		const char* prefix = "MCS_Plunge_face";   //前缀
		int lenPrefix = strlen(prefix);

		char* newString = new char[lenPrefix + tagStr.length() + 1];
		strcpy(newString, prefix);

		MCSName = strcat(newString, MCSTagName);
		...
		//2.4 在新添加的加工坐标系下，创建新的几何体

		// 创建新的workpiece几何体
		tag_t  newWorkpiece = NULL_TAG;
		UF_NCGEOM_create("mill_planar", "WORKPIECE", &newWorkpiece);

		const char * WorkpieceName = nullptr;
		const char* suffix = "WORKPIECE_";   //前缀
		...
		tag_t obj_with_name = NULL_TAG;
		char *tempname = nullptr;
		tempname = const_cast<char*>("MCS_MILL");
		UF_NCGROUP_ask_object_of_name(NewRootGroup, tempname, &obj_with_name);
		...
		markId7 = theSession->SetUndoMark(Session::MarkVisibilityInvisible, "Start");
		theSession->SetUndoMarkName(markId7, NXString("Plunge Levels \345\257\271\350\257\235\346\241\206", NXString::UTF8));
